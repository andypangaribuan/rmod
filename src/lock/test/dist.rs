...
async fn test_dist_lock_pg_combined() {
    ...
    let lock1 = dist(key, None).await.expect("Failed to acquire first lock");

    // 2. Test lock collision (should fail or block and timeout)
    let start = Instant::now();
    let lock2_result = dist(key, Some(opt().wait(std::time::Duration::from_millis(500)))).await;
    ...
    let lock3 = dist(key, Some(opt().wait(std::time::Duration::from_millis(100)))).await.expect("Failed to acquire lock after unlock");
    lock3.unlock().await;
    ...
    let lock_many_1 = dist_many(keys.clone(), None).await.expect("Failed to acquire first multi-lock");

    // 2. Test lock collision on one of the keys
    let start2 = Instant::now();
    let lock_many_2_result = dist(keys[0], Some(opt().wait(std::time::Duration::from_millis(500)))).await;
    ...
    // 4. Test re-acquisition after unlock
    let lock_many_3 =
        dist_many(keys, Some(opt().wait(std::time::Duration::from_millis(100)))).await.expect("Failed to acquire multi-lock after unlock");
